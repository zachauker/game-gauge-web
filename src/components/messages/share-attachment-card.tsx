...
const CARD_CLASS =
  "flex items-center gap-3 rounded-lg border border-border bg-foreground/[0.03] px-3 py-2.5 max-w-xs hover:bg-foreground/[0.06] transition-colors motion-reduce:transition-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring";
...
export function ShareAttachmentCard({ message }: { message: Message }) {
  if (message.type === "GAME_SHARE" && message.game) {
    return (
      <Link href={`/games/${message.game.slug}`} className={CARD_CLASS}>
        <Gamepad2 className="h-5 w-5 text-brand-purple shrink-0" aria-hidden="true"/>
  ...
  if (message.type === "LIST_SHARE" && message.list) {
    return (
      <Link href={`/lists/${message.list.id}`} className={CARD_CLASS}>
        <ListIcon className="h-5 w-5 text-brand-amber shrink-0" aria-hidden="true"/>
        <div className="min-w-0">
  ...
  if (message.type === "REVIEW_SHARE" && message.review) {
    return (
      <Link href={`/games/${message.review.game.slug}?tab=reviews`} className={CARD_CLASS}>
        <Star className="h-5 w-5 text-brand-pink shrink-0" aria-hidden="true"/>
        <div className="min-w-0">
  ...
  if (message.type === "ACTIVITY_SHARE" && message.activityEvent) {
    return (
      ...
      >
        <Activity className="h-5 w-5 text-brand-purple shrink-0" aria-hidden="true"/>
        <div className="min-w-0">
  ...
  return <p className="text-sm text-foreground/60 italic">Shared content unavailable</p>;
}
